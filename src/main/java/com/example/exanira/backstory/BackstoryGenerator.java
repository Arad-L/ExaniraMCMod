package com.example.exanira.backstory;

import com.example.exanira.character.CharacterCreationDefs;
import com.example.exanira.character.LifestyleOption;
import com.example.exanira.character.Profession;
...
public final class BackstoryGenerator {
    ...
    public static String generate(int professionOrdinal, List<Integer> lifestyleChoices) {
        ...
        String physicalBg = getOptionText(lifestyleChoices, 0,
                "stayed active in whatever way you could");

        // Q3 (index 2) — spare-time hobby
        String hobby = getOptionText(lifestyleChoices, 2,
        ...
        return String.format(
                ...
                profession.displayName().toLowerCase(), location,
                physicalBg, hobby, loss, distrust
        );
    }

    private static String getOptionText(List<Integer> choices, int questionIndex, String fallback) {
        if (questionIndex >= choices.size()) return fallback;
        List<LifestyleOption> options = CharacterCreationDefs.QUESTIONS.get(questionIndex).options();
        int choiceIdx = choices.get(questionIndex);
        if (choiceIdx < 0 || choiceIdx >= options.size()) return fallback;
        return options.get(choiceIdx).backstoryText();
    }

    private static <T> T pick(List<T> list, RandomSource random) {
